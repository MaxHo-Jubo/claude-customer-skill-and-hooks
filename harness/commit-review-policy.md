...
4. **SubagentStop（`hooks/subagent-review-clear.ts`）**：**只記錄 `agent_type` 到 debug log，不再清除 marker**（2026-08-17 移除清除職責）。舊行為「型別含 review 就清」在 Tier 3 並行 6 個 agent 時，第一個完成的即解除閘門，其餘面向與 Critical 修復都被放行；full review 的面向 agent（`pr-1134-full-review` 等）亦誤命中。判定依據（型別含 review）與待判定事實（chain 是否跑完）不對應，故整個移除而非調整 pattern。改由 commit-review skill §5 在「面向收齊 + Critical 處理完」後顯式清除，**手動 `bun ~/.claude/scripts/clear-pending-review.ts --aspects-done=<N>` 仍是權威解鎖方式；面向未跑滿時改用 `--force="<理由>"`（會記入 unlock-audit.log）。**

四個 marker 讀寫端（post-commit-review / commit-gate-guard / stop-review-guard / clear-pending-review）共用 `scripts/lib/review-marker.ts`——marker 路徑推導、`git -C`/`cd` 跨 repo 目標解析、`isGitCommitCommand` 指令偵測、`aspectsForTier` 面向數映射、`readMarkerRaw` shape guard。另 `compute-tier.ts` 只借 `resolveRepoRoot`、`subagent-review-clear.ts` 只借 `MARKER_DIR` 寫 debug log，兩者不碰 marker；Tier 判定共用 `scripts/lib/tier.ts`，與 commit-review skill 手動模式（`scripts/compute-tier.ts`）同一份，確保被動／手動判定不分歧。所有 hook 失敗一律 fail-open，不因自身錯誤誤擋正常 commit。手動解鎖：`bun ~/.claude/scripts/clear-pending-review.ts --aspects-done=<N>`；降級時 `--force="<理由>"`。裸執行會因缺聲明而拒絕，這是刻意的。
