...
> 建立：Fable 5，2026-07-03（M4 機器）；本機化：Sonnet 5，2026-07-03（寫入點清單改本機實況）。本檔自身屬黃區。
...
| `~/.claude/rules/**` | 全域規則 |
| `~/.claude/harness/*.md` | 本套制度檔 |
| `<專案>/CLAUDE.md` | 專案入口 |
| `~/.claude/skills/**` | 自訂 skill（修 skill 本身而非堆 workaround memory——root cause 在 skill 就修 skill） |
...
| plugin 啟用狀態與 plugin 內部檔案 | claude-mem / context-mode / superpowers 等 |
...
這台機器上「教訓/知識」有多個活躍寫入點（auto-memory、claude-mem observations、tasks/lessons.md、三層 CLAUDE.md），**手動寫入規則收斂為兩條**：
...
**不要主動寫入**：claude-mem observations 是 hook 自動記錄的系統，模型不手動餵資料；CLAUDE.md 不是教訓收集箱，只有經 §3 抽象化後的「原則」才可提案進 rules/ 或 harness/。
...
**寫入時機**（機械判準）：user 用任何形式糾正你（「不對」「不是這樣」「我說過…」）→ 當下就寫，不等 session 結尾。root cause 若在 harness 規則或 skill 本身寫錯 → 走 §1 黃區提案修規則/skill，而不是堆一條 workaround 教訓。

**壓縮前 flush**：收到 PreCompact 提示（settings.json 已掛 pre-compact-snapshot hook）或自覺 context 即將被壓縮時，先檢查本輪是否有未存的糾正/決策/偏好，寫入後再讓壓縮發生。

## 3. 精簡與抽象化的觸發條件（量化）

任一成立 → 在當前任務收尾後執行精簡（或建議 user 跑 /weekly-review 步驟 5）：
...
【完美正例】「claude-mem 中文化只 patch cache/ 失效」「minified 函式名每版變」「plugin 更新清空 modes/」多條 → 合併為一份 translation-mapping.md 流程文件 + apply-tc.sh 冪等腳本，原零散教訓收斂到單一入口。
【典型反例】把三條互不相關的坑硬塞進一條「要小心 plugin」——抽象到失去觸發信號的教訓等於刪除。
...
### 禁止事項（任何等級模型皆不可）
- 不可刪除或弱化 judgment-matrix.md §3 熔斷條件——只可經 user 同意後**收緊**或修正錯誤
