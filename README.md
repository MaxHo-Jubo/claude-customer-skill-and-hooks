...
| spec-design | `/spec-design` | 1.2.0 | 從需求到設計 spec：需求釐清 → 方案比較 → 撰寫 spec → 4-agent review → 迭代修正 |
| plan-and-execute | `/plan-and-execute` | 1.1.0 | 從設計 spec 到完成實作：Plan → TDD → 分 Wave 實作 → 驗證 |
...
### 2026-03-27: 新增 spec-design v1.2.0、plan-and-execute v1.1.0

**Skills 新增：**
- `spec-design`（v1.2.0）— 從需求到設計 spec 的結構化流程：需求釐清 → 方案比較 → 設計呈現 → 撰寫 spec → 4-agent review → 迭代修正。支援 superpowers 自動 delegate
- `plan-and-execute`（v1.1.0）— 從設計 spec 到完成實作：Plan → TDD → 分 Wave 實作 → 驗證。支援 superpowers delegate 與原生流程雙路徑

**CLAUDE.md 更新：**
- 更新至 `CLAUDE.md.20260327`
