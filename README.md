...
| Skill | 指令 | 版本 | 用途 |
|-------|------|------|------|
| jira | `/jira` | 1.0.0 | Jira Issue 管理，自動從 branch 識別 issue |
...
| test-module | `/test-module <path>` | 1.0.0 | 掃描可測試函式，產出單元測試 |
| explore-report | `/explore-report <dir>` | 1.0.0 | 探索目錄並強制產出結構化報告 |
| method-refactor | `/method-refactor <method>` | 1.0.0 | 7 項檢查結構化優化重構方法 |
| weekly-review | `/weekly-review` | 1.0.0 | 每週工作回顧、記憶整理，整合 skill 錯誤 pattern 分析與修補建議（8 步） |
| sync-my-claude-setting | `/sync-my-claude-setting` | 1.0.0 | 同步本機 Claude 設定到 Repo |
...
| PostToolUse | git commit 後 | Bash | `post-commit-review.cjs` | 提醒 Claude 執行 POST-COMMIT-REVIEW 規則 |
...
| MCP Servers | 0 | `mcp-servers.json` 目前為空（MCP Servers 由 plugins 自行管理） |

## MCP Servers 一覽

> `mcp-servers.json` 目前為空物件 `{}`。以下 MCP Servers 由啟用的 plugins 自行註冊與管理：
...
| MCP Servers 設定 | `~/.claude/mcp-servers.json`（目前為空，由 plugins 管理） |
