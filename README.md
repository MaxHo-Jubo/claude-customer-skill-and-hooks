...
| weekly-review | `/weekly-review` | 每週工作回顧、記憶整理，整合 skill 錯誤 pattern 分析與修補建議（8 步） |
...
| ~~PostToolUse~~ | ~~git commit 後~~ | ~~Bash~~ | ~~`post-commit-review.cjs`~~ | ~~已移除，改為 CLAUDE.md POST-COMMIT-REVIEW 規則（2026-03-16）~~ |
...
| 分類 | 數量 | 說明 |
|------|------|------|
| Plugins（啟用） | 13 | code-review、atlassian、frontend-design、claude-md-management、typescript-lsp、gopls-lsp、jdtls-lsp、context7、claude-mem、context-mode、document-skills、superpowers、claude-hud |
| Plugins（停用） | 3 | code-simplifier、github、everything-claude-code |
| MCP Servers | 5 | context7、context-mode、mcp-search、atlassian、typescript-lsp（另有 claude-hud 為 marketplace plugin） |

## MCP Servers 一覽

| Server | 類型 | Command | 用途 |
|--------|------|---------|------|
| context7 | stdio | `npx @upstash/context7-mcp` | 即時查詢第三方套件文件，避免使用過時 API |
| gitlab | http | `gitlab.webotopia.work` | GitLab 整合，存取 MR、Issue、Pipeline 等 |
| gitnexus | stdio | `npx gitnexus mcp` | 程式碼知識圖譜 MCP 介面，結構化查詢 codebase |
...
| MCP Servers 設定 | `~/.claude/mcp-servers.json` |
