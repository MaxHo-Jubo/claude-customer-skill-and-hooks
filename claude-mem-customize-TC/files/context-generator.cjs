...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",l={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},_=new W;var Ot={};function bt(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,ce.fileURLToPath)(Ot.url))}var ht=bt(),C=I.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),Bt=(0,b.join)(y,"plugins","marketplaces","thedotmack"),Ht=(0,b.join)(C,"archives"),Wt=(0,b.join)(C,"logs"),Yt=(0,b.join)(C,"trash"),Vt=(0,b.join)(C,"backups"),qt=(0,b.join)(C,"modes"),Kt=(0,b.join)(C,"settings.json"),_e=(0,b.join)(C,"claude-mem.db"),Jt=(0,b.join)(C,"vector-db"),Qt=(0,b.join)(C,"observer-sessions"),zt=(0,b.join)(y,"settings.json"),Zt=(0,b.join)(y,"commands"),es=(0,b.join)(y,"CLAUDE.md");function me(r){(0,pe.mkdirSync)(r,{recursive:!0})}function le(){return(0,b.join)(ht,"..")}var $=class{db;constructor(e=_e){e!==":memory:"&&me(C),this.db=new ue.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys()}initializeSchema(){this.db.run(`
    ...
    `);let e=this.db.prepare("SELECT version FROM schema_versions ORDER BY version").all();(e.length>0?Math.max(...e.map(s=>s.version)):0)===0&&(_.info("DB","Initializing fresh database with migration004"),this.db.run(`
        ...
        CREATE TABLE IF NOT EXISTS 筆觀察（
          id INTEGER PRIMARY KEY AUTOINCREMENT,
...
`).filter(n=>n.trim()),s="";for(let n=t.length-1;n>=0;n--)try{let o=t[n];if(!o.includes('"type":"assistant"'))continue;let i=JSON.parse(o);if(i.type==="assistant"&&i.message?.content&&Array.isArray(i.message.content)){let a="";for(let d of i.message.content)d.type==="text"&&(a+=d.text);if(a=a.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),a){s=a;break}}}catch(o){_.debug("PARSER","Skipping malformed transcript line",{lineIndex:n},o);continue}return{userMessage:"",assistantMessage:s}}catch(e){return _.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:r},e),{userMessage:"",assistantMessage:""}}}function z(r,e,t,s){if(!e.showLastMessage||r.length===0)return{userMessage:"",assistantMessage:""};let n=r.find(d=>d.memory_session_id!==t);if(!n)return{userMessage:"",assistantMessage:""};let o=n.memory_session_id,i=Ct(s),a=be.default.join(y,"projects",i,`${o}.jsonl`);return Nt(a)}function Re(r,e){let t=e[0]?.id;return r.map((s,n)=>{let o=n===0?null:e[n+1];return{...s,displayEpoch:o?o.created_at_epoch:s.created_at_epoch,displayTime:o?o.created_at:s.created_at,shouldShowLink:s.id!==t}})}function Z(r,e){let t=[...r.map(s=>({type:"observation",data:s})),...e.map(s=>({type:"summary",data:s}))];return t.sort((s,n)=>{let o=s.type==="observation"?s.data.created_at_epoch:s.data.displayEpoch,i=n.type==="observation"?n.data.created_at_epoch:n.data.displayEpoch;return o-i}),t}function Ce(r,e){return new Set(r.slice(0,e).map(t=>t.id))}function Ne(){let r=new Date,e=r.toLocaleDateString("en-CA"),t=r.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0}).toLowerCase().replace(" ",""),s=r.toLocaleTimeString("en-US",{timeZoneName:"short"}).split(" ").pop();return`${e} ${t} ${s}`}function Ae(r){return[`# [${r}] 近期脈絡， ${Ne()}`,""]}function Ie(){return[`**圖例：** 工作階段請求 | ${O.getInstance().getActiveMode().observation_types.map(t=>`${t.emoji} ${t.id}`).join(" | ")}`,""]}function ye(){return["**欄位說明**：","- **讀取**：讀取此觀察所需的 token 數（當前學習成本）","- **研究**：產生此記錄所花費的 token 數（研究、建構、決策）",""]}function Me(){return["**脈絡索引：** 此語義索引（標題、類型、檔案、token 數）通常足以理解過去的工作。","","當需要實作細節、決策理由或除錯脈絡時：","- 使用 MCP 工具（search、get_observations）按需獲取完整觀察","- 關鍵類型（bugfix、decision）通常需要獲取詳細內容","- 信任此索引而非重新閱讀程式碼來回顧過去的決策與經驗",""]}function Le(r,e){let t=[];if(t.push("**脈絡經濟**："),t.push(`- 載入： ${r.totalObservations} 筆觀察（${r.totalReadTokens.toLocaleString()} 個 token 待讀取）`),t.push(`- 研究投入： ${r.totalDiscoveryTokens.toLocaleString()} 個 token 花費在研究、建構和決策上`),r.totalDiscoveryTokens>0&&(e.showSavingsAmount||e.showSavingsPercent)){let s="- 節省：";e.showSavingsAmount&&e.showSavingsPercent?s+=`${r.savings.toLocaleString()} tokens (${r.savingsPercent}% 透過復用節省)`:e.showSavingsAmount?s+=`${r.savings.toLocaleString()} tokens`:s+=`${r.savingsPercent}% 透過復用節省`,t.push(s)}return t.push(""),t}function ve(r){return[`### ${r}`,""]}function De(r){return[`**${r}**`,"| ID | 時間 | 類 | 標題 | 讀取 | 研究 |","|----|------|---|-------|------|------|"]}function xe(r,e,t){let s=r.title||"無標題",n=O.getInstance().getTypeIcon(r.type),{readTokens:o,discoveryDisplay:i}=M(r,t),a=t.showReadTokens?`~${o}`:"",d=t.showWorkTokens?i:"";return`| #${r.id} | ${e||'"'} | ${n} | ${s} | ${a} | ${d} |`}function Ue(r,e,t,s){let n=[],o=r.title||"無標題",i=O.getInstance().getTypeIcon(r.type),{readTokens:a,discoveryDisplay:d}=M(r,s);n.push(`**#${r.id}** ${e||'"'} ${i} **${o}**`),t&&(n.push(""),n.push(t),n.push(""));let c=[];return s.showReadTokens&&c.push(`Read: ~${a}`),s.showWorkTokens&&c.push(`Work: ${d}`),c.length>0&&n.push(c.join(", ")),n.push(""),n}function ke(r,e){let t=`${r.request||"工作階段開始"} (${e})`;return[`**#S${r.id}** ${t}`,""]}function x(r,e){return e?[`**${r}**: ${e}`,""]:[]}function we(r){return r.assistantMessage?["","---","","**先前**","",`A: ${r.assistantMessage}`,""]:[]}function $e(r,e){return["",`Access ${Math.round(r/1e3)}k tokens of 的過往研究與決策，僅需 ${e.toLocaleString()}t. 使用 MCP 搜尋工具按 ID 存取記憶。`]}function Fe(r){return`# [${r}] 近期脈絡， ${Ne()}

此專案尚未找到先前的工作階段記錄。`}function Pe(){let r=new Date,e=r.toLocaleDateString("en-CA"),t=r.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0}).toLowerCase().replace(" ",""),s=r.toLocaleTimeString("en-US",{timeZoneName:"short"}).split(" ").pop();return`${e} ${t} ${s}`}function Xe(r){return["",`${p.bright}${p.cyan}[${r}] 近期脈絡， ${Pe()}${p.reset}`,`${p.gray}${"\u2500".repeat(60)}${p.reset}`,""]}function je(){let e=O.getInstance().getActiveMode().observation_types.map(t=>`${t.emoji} ${t.id}`).join(" | ");return[`${p.dim}圖例：工作階段請求 | ${e}${p.reset}`,""]}function Ge(){return[`${p.bright}欄位說明${p.reset}`,`${p.dim}  讀取：讀取此觀察所需的 token 數（當前學習成本）${p.reset}`,`${p.dim}  研究：產生此記錄所花費的 token 數（研究、建構、決策）${p.reset}`,""]}function Be(){return[`${p.dim}脈絡索引：此語義索引（標題、類型、檔案、token 數）通常足以理解過去的工作。${p.reset}`,"",`${p.dim}當需要實作細節、決策理由或除錯脈絡時：${p.reset}`,`${p.dim}  - 使用 MCP 工具（search、get_observations）按需獲取完整觀察${p.reset}`,`${p.dim}  - 關鍵類型（bugfix、decision）通常需要獲取詳細內容${p.reset}`,`${p.dim}  - 信任此索引而非重新閱讀程式碼來回顧過去的決策與經驗${p.reset}`,""]}function He(r,e){let t=[];if(t.push(`${p.bright}${p.cyan}脈絡經濟${p.reset}`),t.push(`${p.dim}  載入： ${r.totalObservations} 筆觀察（${r.totalReadTokens.toLocaleString()} 個 token 待讀取）${p.reset}`),t.push(`${p.dim}  研究投入： ${r.totalDiscoveryTokens.toLocaleString()} 個 token 花費在研究、建構和決策上${p.reset}`),r.totalDiscoveryTokens>0&&(e.showSavingsAmount||e.showSavingsPercent)){let s="  節省：";e.showSavingsAmount&&e.showSavingsPercent?s+=`${r.savings.toLocaleString()} tokens (${r.savingsPercent}% 透過復用節省)`:e.showSavingsAmount?s+=`${r.savings.toLocaleString()} tokens`:s+=`${r.savingsPercent}% 透過復用節省`,t.push(`${p.green}${s}${p.reset}`)}return t.push(""),t}function We(r){return[`${p.bright}${p.cyan}${r}${p.reset}`,""]}function Ye(r){return[`${p.dim}${r}${p.reset}`]}function Ve(r,e,t,s){let n=r.title||"無標題",o=O.getInstance().getTypeIcon(r.type),{readTokens:i,discoveryTokens:a,workEmoji:d}=M(r,s),c=t?`${p.dim}${e}${p.reset}`:" ".repeat(e.length),m=s.showReadTokens&&i>0?`${p.dim}(~${i}t)${p.reset}`:"",l=s.showWorkTokens&&a>0?`${p.dim}(${d} ${a.toLocaleString()}t)${p.reset}`:"";return`  ${p.dim}#${r.id}${p.reset}  ${c}  ${o}  ${n} ${m} ${l}`}function qe(r,e,t,s,n){let o=[],i=r.title||"無標題",a=O.getInstance().getTypeIcon(r.type),{readTokens:d,discoveryTokens:c,workEmoji:m}=M(r,n),l=t?`${p.dim}${e}${p.reset}`:" ".repeat(e.length),T=n.showReadTokens&&d>0?`${p.dim}(~${d}t)${p.reset}`:"",E=n.showWorkTokens&&c>0?`${p.dim}(${m} ${c.toLocaleString()}t)${p.reset}`:"";return o.push(`  ${p.dim}#${r.id}${p.reset}  ${l}  ${a}  ${p.bright}${i}${p.reset}`),s&&o.push(`    ${p.dim}${s}${p.reset}`),(T||E)&&o.push(`    ${T} ${E}`),o.push(""),o}function Ke(r,e){let t=`${r.request||"工作階段開始"} (${e})`;return[`${p.yellow}#S${r.id}${p.reset} ${t}`,""]}function U(r,e,t){return e?[`${t}${r}:${p.reset} ${e}`,""]:[]}function Je(r){return r.assistantMessage?["","---","",`${p.bright}${p.magenta}先前${p.reset}`,"",`${p.dim}A: ${r.assistantMessage}${p.reset}`,""]:[]}function Qe(r,e){let t=Math.round(r/1e3);return["",`${p.dim}Access ${t}k tokens of 的過往研究與決策，僅需 ${e.toLocaleString()}t. 使用 MCP 搜尋工具按 ID 存取記憶。${p.reset}`]}function ze(r){return`
${p.bright}${p.cyan}[${r}] 近期脈絡， ${Pe()}${p.reset}
${p.gray}${"\u2500".repeat(60)}${p.reset}

${p.dim}此專案尚未找到先前的工作階段記錄。${p.reset}
